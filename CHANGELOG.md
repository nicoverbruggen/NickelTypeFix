...
- **Page-boundary probe** (`ntf_pagecut_probe`, **off** by default): a diagnostic for the reported "characters cut off at the top or bottom of a page" issue in kepub books. When switched on, the mod logs how the reader places each page boundary (the inputs and result of every page cut) so the defect can be pinned down from a normal reading session. It observes only: pages are still cut exactly as without the mod, and with the key at its default the probe does nothing at all.
- **Page-boundary clipping fix** (`ntf_pagecut_fix`, **off** by default while field validation runs): the probe found the cause of the cut-off letters — a kepub page boundary can land inside a line, exactly in the overlap between one line's box and the next, so the next page's first line loses the tops of its letters and they paint as a thin strip at the bottom of the previous page. With this on, a boundary that would cut through a line of your sideloaded reading font is moved into the empty gap between that line's letters and the next line's, measured from the font's real letter shapes. A boundary that is already clean is left byte-identical, and anything the fix cannot prove safe (headings, images, built-in fonts it cannot measure, vertical books, publisher-default fonts) keeps stock behaviour. Where it engages, page counts can shift by a page once; your reading position is kept. It stays off by default until it has been validated in the field; switch it on with `ntf_pagecut_fix:1` (a sideloaded reading font must be selected).
