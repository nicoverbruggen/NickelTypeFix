...
- **A centred image stays centred when you set the text alignment** (`ntf_center_images`, on by default): with the reader's text alignment set to left, or to justified, a figure the book had centred moved to the left margin. Kobo applies your alignment setting to every block on the page, and an image alone in a block is positioned by that block's alignment, so the book's own centring was overridden along with the text. The mod now reads what the book's own stylesheet says about each of those blocks and puts the centring back where the author asked for it, and only there. An image the author left-aligned, or never styled, is untouched, and so is one that is already sitting centred on the page, so with your alignment left at the publisher's default this does nothing at all. Turn it off with `ntf_center_images:0`.
- **Oversized drop caps no longer push the line under them down** (`ntf_dropcap_fix`, on by default): a large initial letter at the start of a chapter took part in its own line and made that line taller, so the first two lines of the paragraph sat visibly further apart than every pair after them (88 px against 69 px on the device it was measured on). The drop cap is now stopped from inflating its line, and the reader works out where the chapter's pages end from the corrected layout rather than from the layout before it. A drop cap the book floats is already correct, because a float sits beside the text instead of inside the line, and those are left exactly as the book set them. Turn it off with `ntf_dropcap_fix:0`.
- **Two diagnostics for bug reports**, both off by default: `ntf_pagecut_probe` logs the kepub line boxes and where each page boundary landed, and `ntf_page_probe` logs what a page actually contains. They only observe. With any of them on, a page is laid out and cut exactly as it is with them off; only the log gains detail. Leave them at `0` unless a bug report asks for one.
