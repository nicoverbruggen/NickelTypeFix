...
- **Config mistakes now explain themselves**: a misspelled setting, a malformed line, or an invalid value in the config file is warned about in the log, and full verbose logging switches on automatically for that boot — so if a setting doesn't seem to take effect, the log tells you why.
