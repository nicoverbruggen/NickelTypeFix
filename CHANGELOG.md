...
- **Two development-build diagnostics for bug reports**: the page-boundary probe logs the kepub line boxes and where each page boundary landed, and the page probe logs what a page actually contains. Development builds always run both probes while the mod is enabled. Release builds omit their hooks and code entirely. Both probes only observe and leave pagination unchanged.
