...
static int ntf_pagecut_trim_rects(QVector<QRect> *rects, int *guard_skips) {
...
// FIX 9 — the trim. sortRectsByStart's one caller sits past locatePages' early exits and right
// before the page walk, so the vector it just sorted is exactly the geometry the walk will read.
// The real sort runs first and the trim then mutates the sorted vector in place. Every ungated
// case leaves the vector untouched, and the try/catch keeps a Qt allocation failure from
// unwinding out of an extern "C" hook.
extern "C" __attribute__((visibility("default")))
void *_ntf_wv_sortRects(QVector<QRect> *rects, int dir) {
    ...
    if (ntf_enabled() && ntf_pagecut_trim() && dir == 0 && rects
        ...
        try {
            int refused = 0;
            int trimmed = ntf_pagecut_trim_rects(rects, &refused);
