...
NickelTypeFix corrects ten text-rendering defects in Kobo's Qt 5.2 / QtWebKit / Monotype iType reader stack (firmware 4.x). Each fix is **independent**: it engages only if its seam is present on the running firmware, and otherwise logs and sits out. A mismatch in one fix never affects the others. An eleventh fix is written up here too, and ships off by default because it is not finished (see [Fix 11](fix-11-oversized-drop-caps-ntf_dropcap_fix-off-by-default-unfinished)).
...
**The fix.** Put the author's centring back, on the blocks that had it and no others. The script (see [Script in the book's frame](#script-in-the-books-frame)) walks the images in the chapter, climbs out of the koboSpans the reader wraps content in to reach the real block, and requires that block to hold the image and nothing else: one element child, no text. It then asks what the *book* said about that block, reading the matched rules with `getMatchedCSSRules` and skipping the reader's own injected selector, and where the author asked for centre it sets `text-align: center` on the block as an inline style with `important` priority. That outranks the injected rule, so the figure centres again.

It sets no width, no margin and no `display`, on purpose. Centring by `display: block` plus auto margins would also work and would be immune to `text-align` altogether, but it changes the image's formatting context and reflows everything under it, which is the failure Fix 11 ran into. This changes no box's size, so nothing below the image moves and a page table built before the script ran still describes the document. An image the author left-aligned, or never styled at all, is left alone; so is one that shares its block with text. Each image is marked once and skipped afterwards. Confirmed on device.

---

## Fix 11 — Oversized drop caps · `ntf_dropcap_fix` (off by default, unfinished)
...
**The fix, and why it is off.** The span is clamped to `display: inline-block` with a height and a line-height of one em, which takes it out of the line-box maximum while leaving the text wrap alone. Floating it would work too, but a float indents the lines beside it and changes the design the author chose. The script only touches a paragraph's first element child, a `span` that is not a koboSpan, holding at most two characters, whose computed font size is more than 1.6 times the paragraph's. On the page, this works: the spacing comes out right and the drop cap looks the way it should.

It ships off because of what it does to pagination. Clamping the drop cap makes the paragraph shorter, and the reader has already worked out where its pages end from the layout as it was. Crossing backwards into such a chapter then positions the view against a page table that no longer matches the document, and the page opens mid-line, with the bottom of a row of glyphs along the top. Bisected on device: the drop cap on its own reproduces it, Fix 10 on its own never does, and the difference between them is that Fix 10 changes no box sizes. Running the script earlier is not the answer either; it already runs at the last point before pagination measures anything. The fix stays off until the spacing can be corrected without changing the layout after the page table has been built. `ntf_dropcap_fix:1` gets the spacing and the paging problem together.
...
**When.** At the end of the `addCssToHtml` hook, after the real call has put the reader's CSS into the live document. The ordering probe (`ntf_order_probe`) was written to settle this on hardware rather than assume it, and it showed every pagination that matters is immediately preceded by that call, both on chapter load and after a settings change. A settings change re-paginates with no `loadFinished` at all, so hooking chapter load would leave the page uncorrected until the next chapter. Running at this point puts the script's changes in place before `locatePages` measures anything.

**What it does.** It reads the chapter and sets inline styles on the elements it can identify: `text-align` on a block holding a centred image, and `display`, `height` and `line-height` on a drop-cap span. That is all it writes. It adds no elements, reads nothing from outside the page, sends nothing anywhere, and never touches the book's files on disk. Because it sits on the path of every re-render it runs several times per chapter, so it has to be idempotent: it marks each element it touches with a `data-ntf` attribute, skips a marked element next time, and returns immediately when there is nothing to do. With both fixes and the page probe off, no script is built and none runs.
