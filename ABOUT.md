...
NickelTypeFix corrects eight text-rendering defects in Kobo's Qt 5.2 / QtWebKit / Monotype iType reader stack (firmware 4.x), and ships Fix 9, a page-boundary diagnostic probe plus its corrective stage (both off by default). Each fix is **independent**: it engages only if its seam is present on the running firmware, and otherwise logs and sits out. A mismatch in one fix never affects the others.
...
## Fix 9 — kepub page-boundary clipping · `ntf_pagecut_probe` (stage 1) / `ntf_pagecut_fix` (stage 2)

**The bug.** Depending on the font, font size, and line spacing, letters on the first or last line of a kepub page can be shaved at the page edge: flattened ascenders and question marks at the top of a page, or clipped descenders at the bottom. The missing ink is painted on the neighbouring page instead (a thin strip of glyph tops at the bottom of the previous page).

**Mechanism.** The kepub reader builds its page table from one rectangle per text run: `WebkitView::locatePages` collects the run rects, sorts them (`WebkitView::sortRectsByStart`), merges adjacent ones, and walks them page by page. On most iterations the last run rect that starts within the page limit also ends within it, and the next page boundary is placed without any cutting; when a rect straddles the limit, the walk calls the static `WebkitView::cutPage(rects, start, limit, direction)`, which returns the deepest run-rect bottom that still fits, falling back to the raw limit when none fits. The next page starts exactly at the placed boundary, and the paint is clipped to it. The run rect spans `baseline - ascent` to `baseline + descent` plus a small measured slack, but real glyph ink can extend outside that span at either end, and whatever falls outside lands on the wrong side of the boundary. Whether a given font clips, and at which edge, depends on where the boundary lands inside the gap between two lines' ink.

**Stage 1: measure before touching anything.** A wrong page boundary is a rendering bug; a wrongly *moved* page boundary could hang pagination, so the corrective stage is gated on measurements only a live reader can provide: how often each boundary path runs in real books, what the run-rect geometry looks like per firmware, and what unit the reader's font-size value is in. This stage hooks the pagination seams — `cutPage`, `sortRectsByStart`, and the two `locatePages` entry points — as pure passthroughs: the real functions always run and their results are returned unchanged, so pagination with the probe on is byte-identical to stock. With `ntf_pagecut_probe:1` the mod logs, per pagination pass, the view (tagged when it is the reader's own view — `locatePages` is virtual, so the reader cannot be identified by hooking its override), the reader's font size, the sorted run-rect geometry, the resulting page count, each placed page boundary read back from the finished page table and classified against the line rects (on a rect's top, on a rect's bottom, or elsewhere), and for each cut the raw arguments, the returned boundary, and whether that boundary was a run-rect bottom or the raw limit, with per-pass caps so a long book cannot flood the log. An observation the probe cannot attribute to a pass (an unexpected thread, a cut with no pagination running) is still logged as a capped "stray" line rather than dropped, so an empty log is evidence of absence, not of a blind spot. With the key at its default (`0`) the probe does nothing.

**Stage 2: the fix · `ntf_pagecut_fix` (off by default while field validation runs).** The probe's device data pinned the defect to the `cutPage` path: consecutive line rects overlap (a rect is `ascent + descent + ~0.135 em` tall against a smaller line advance), so the "deepest rect bottom that fits" lands inside the next line's rect — exactly `height − advance` pixels inside it, 3 px on the measured firmware, on every cut. The next page then starts with the tops of its first line's letters shaved off, and the missing ink paints as a thin strip at the bottom of the previous page. The fix hooks the same `cutPage` call and clamps its return into the measured **ink gap**: no higher than the boundary line's deepest real descender, no lower than the next line's tallest real ascender. Ink extents are measured per font family at registration, from the font file's own outline bounding boxes over a fixed repertoire (printable ASCII plus Latin-1 letters), and the reading family is taken from the reader's own per-chapter CSS injection. A boundary already inside the gap is returned unchanged, so pagination is byte-identical wherever it was already correct.

It never guesses. The clamp applies only when: the pass is the reader's own view (same identity proof as Fix 6); the injected reading family has measured metrics (TrueType-outline files only — a CFF, collection, or compressed built-in font leaves the family unmeasured); the pass is horizontal; the boundary is verifiably a run-rect bottom; the two lines around it match the font's predicted body-text height within 2 px; and the ink gap is non-empty. Every other case — headings, images, fallback-font lines, vertical books, publisher-default fonts, the raw-limit fallback path, an ambiguous or unrecognized boundary — keeps the stock boundary, so the failure mode of the whole feature is today's behaviour. The adjusted value is also range-checked against the pagination loop's invariants before it is used. The no-cut boundary path (a break placed exactly on a line's rect top) is deliberately out of scope for this stage: for the measured fonts its worst error is the sub-pixel ascender overhang, an order smaller than the cut path's. Boundaries can move by a few pixels where the fix engages, so page counts can shift once when it is switched on; reading positions are stored as anchors into the text rather than page numbers, so they are expected to survive (checked against the bookmark schema, not yet end-to-end on device).
