...
NickelTypeFix corrects defects in Kobo's Qt 5.2 / QtWebKit / Monotype iType reader stack (firmware 4.x): most in text rendering, some in how fast a chapter opens. Each fix has its own configuration switch and checks its required targets. Some fixes share hooks or detours, so a missing target can disable several features. Passing these checks does not prove correct behavior for every font, book, or firmware.
...
**How boot recovery works.** NickelHook renames the plugin out of its load path before installing hooks. After initialization succeeds, it starts a three-second timer to restore the filename. If Nickel exits before that restore, the plugin remains outside its load path on the next boot. A hang during initialization may require a forced restart. The failsafe does not detect errors after the filename is restored. Code patches affect memory, but the mod does write its own config, logs, and recovery markers to disk.
...
The correction pass uses the sorted rectangle tops to limit each search. Boundary snapping looks back by the tallest rectangle's height, which includes every box that could end at the boundary and every overlapping peer. Page fitting and render-end calculation inspect only boxes whose tops belong to that page. The geometry guards still make the same decisions. This avoids scanning the whole chapter again for every page.

In VirtualNickel on Clara Colour firmware 4.46.23836, a synthetic 274-page chapter with 4,924 line rectangles took about 20 ms per correction pass before bounded searches and 0.5 ms after them. Loading ran two passes, removing about 39 ms of correction work. Page-start and render-end hashes matched. These are emulated timings for that fixture, not a measured hardware speedup or a prediction for every book.
...
For non-design metrics, old HarfBuzz rounds GPOS advance adjustments before adding them to the font's base advances. NG leaves these adjustments fractional. Qt then floors each absolute glyph position when drawing, so the same word can have different one-pixel gaps at different horizontal positions. The mod applies the old adjustment rounding to NG runs with advancing glyphs and no positioning offsets. It preserves the base metrics, design metrics, glyph IDs and clusters. Runs with attached marks, offsets, vertical advances or right-to-left text keep their original positioning.

The cache then detours the selected shaper's prologue. On a miss it runs the shaper and the mod's justification and small-caps processing, then records the result; on a hit it replays that result. Replay is intended to preserve glyphs and positions for the same inputs. The [ARM regression suite](test/rendering/README.md) compares cached and uncached results separately under each shaper; it does not establish equivalence between the two shapers. The key includes the font definition and engine type, text, kerning flag, paragraph direction, script, sub-font index, item boundaries, small-caps state, and design-metrics mode. Using the font definition rather than an engine pointer avoids confusing a reused allocation with its previous font. A lock guards the table because multiple threads can shape text. Records are capped, items longer than 96 characters are not cached, and a replay is skipped when the glyph buffer has no room.

**Repeated line preparation.** Once shaping is cheap, Qt's surrounding layout work becomes visible. `QTextLine::cursorToX` calls `QTextEngine::shapeLine` for each measured position. That function visits every text item on the line and accumulates its width even when all its glyphs are already ready. In Qt 5.2, that width serves only to calculate tab stops. Ordinary ready items do no work in `shapeText`.

The mod detours `shapeLine` and returns immediately when every item in the line already has glyphs and none is a tab or inline object. Tabs still need width calculation, and objects can resize during preparation, so both keep the original path. It checks the current items on every call and retains no layout pointer or readiness result. This shortcut uses the existing `ntf_fast_shaping` switch, works with either shaper, and sits out independently if its symbol or detour is unavailable. The ARM tests compare wrapping, metrics, cursor positions, and pixels before and after installing it under both shapers.
...
**Font dropdown repair.** The NG flag also changes Qt's text itemization outside WebKit. With the stock shaper, a Latin font name such as `KF Libron` is one item. NG separates the words and space. Qt's rich-text font-engine cache keeps only the last item, matched by position, length and script. After Nickel destroys the menu rows and unregisters their fonts, the single-item label can retain its font engine. The multi-item label needs another font lookup during repaint and resolves to a fallback. Reloading the font does not clear the fallback retained by the label's text format. This was reproduced under ARM Qt 5.2.1 without any of the mod's fixes, and the device traces show the same sequence.

The mod captures the selected label's pixels immediately after Nickel changes its HTML through the `QLabel::setText` import, while the preview font is still registered. A temporary event filter paints those pixels through menu cleanup, so the missing font cannot flash on screen. After `adjustFontFamily` returns, when Nickel has reloaded the chosen font, the mod removes the filter and rebuilds the label. It matches the family and the `fontFaceComboBox` ancestor and excludes popup rows. A changed label text bypasses the saved image; hiding, resizing, or restyling the label discards it. The label owns the filter, so destruction also releases it. No extra delayed timer or private widget offsets are used. NG stays enabled by default. If either hook is unavailable, the mod keeps the stock shaper and can still cache its output. The repair addresses this font-lifetime case; it is not a general guarantee that NG leaves all Nickel UI rendering unchanged.
...
Qt maps uppercase text to glyph IDs before calling either shaper. For a multi-font engine, old HarfBuzz reuses those IDs even when given lowercase text. The fix maps lowercase text through the primary font and replaces only glyph IDs assigned to that font. Fallback glyphs, font boundaries, and line metrics stay as Qt prepared them. A letter missing from the primary font keeps its existing glyph. Mapping uses a temporary buffer so a failed conversion leaves Qt's input intact.
...
**Safety.** The `fontEngine` detour is installed only after the shaper detour is confirmed running; the detour on its own would shape the uppercased text at full size, which is full capitals. A prologue that cannot be relocated sits the fix out with a log line. Every table read is bounds-checked and a malformed table reads as "no small caps", which is the stock outcome. The cache keeps the 32 most recently used font records. Each render holds a shared reference to its record, so eviction does not invalidate data another thread is using. An evicted record is freed after its final reader finishes; revisiting that font rebuilds its record.

## Fix 15 — Pauses between EPUB chunks · `ntf_fast_epub_delivery`

Nickel's `EpubNetworkReply` delivers a local chapter to WebKit in chunks. On 4.46.23836, the first read can return 4 KiB, followed by 48 KiB chunks with a 100 ms timer between them. The reader also has a path that uses larger chunks without a delay when advancing toward a saved position. The sample attached to [performance issue #5](https://github.com/nicoverbruggen/NickelTypeFix/issues/5#issuecomment-5483575501) schedules three 100 ms waits when entering its first text chapter after kepubify's blank page.

The fix hooks Nickel's import of `QTimer::singleShotImpl`. During a tracked chapter load, it changes a 100 ms interval to zero only when the receiver's exact Qt class is `EpubNetworkReply`. Qt still queues the original callback with the same receiver and timer type. It retains callback order, cancellation on receiver destruction, and the original chunk contents and sizes. Other classes, intervals, and calls outside the load window pass through. This does not depend on the shaper or `optimizeLegibility`.

The existing chapter-start and load-finished hooks bound the window. Reader destruction closes it, and the existing 30-second limit prevents an unfinished load from keeping it active indefinitely. The feature has its own switch and requires the timer import plus both load hooks. Missing hooks leave the original delays in place. The ARM test checks interval guards, asynchronous execution, callback order, and receiver destruction.

In VirtualNickel on Clara Colour 4.46.23836, four alternating repeated loads of the issue's 151,447-byte chapter used KF Charis Old Style, left alignment, `readingFontSize=32`, and `optimizeLegibility`. Measurements start when entering page 2, excluding the blank page. With a warm shaping cache, median load time fell from 846 ms to 528 ms when this fix, the line-preparation shortcut, and the bounded page-correction searches were enabled together. Median time before the main layout fell from 382 ms to 129 ms. The line rectangles, page-start and render-end hashes, and captured page-2 pixels matched across all eight runs. The line-preparation shortcut alone reduced median load time from 857 ms to 790 ms in a separate comparison.

These measurements use instrumented firmware under emulation. They do not predict the same percentage improvement on hardware. The 100 ms intervals are fixed, and parsing can overlap part of each interval. The net saving depends on the book, current position, font, and device.
