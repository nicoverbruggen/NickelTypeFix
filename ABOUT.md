...
NickelTypeFix corrects four text-rendering defects in Kobo's Qt 5.2 / QtWebKit / Monotype iType reader stack (firmware 4.x). Each fix is **independent** — it engages only if its seam is present on the running firmware and otherwise logs and sits out, so a mismatch on one never affects the others.

Two of the fixes are **PLT hooks** (via NickelHook: it patches a library's `R_ARM_JUMP_SLOT` GOT entries, so it can intercept a *cross-library call* to an exported symbol). The other two target functions that are **stripped/inlined** in the shipped binaries — there is no symbol to hook — so they are applied as **in-memory byte patches** at startup (see [In-memory patching](#in-memory-patching)). Nothing is written to any device library on disk; every change is made in the process's memory at boot and is gone when the mod is removed. This is interoperability bugfixing: no Kobo, Qt, or Monotype code is redistributed.
...
- The hooks (Fixes 1–2) bind exact symbols and are `optional`; a rename makes that fix inert.
